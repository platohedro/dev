...
export default async function NewsDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-background text-foreground"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: jsonLd(structuredData) }} /><SiteHeader /><article className="mx-auto max-w-4xl px-6 py-16 md:px-10">{item.cover_image_url && <img src={mediaUrl(item.cover_image_url)} alt={`Imagen de ${item.title}`} className="mb-8 max-h-[32rem] w-full object-cover" />}<p className="text-sm text-muted-foreground">{new Intl.DateTimeFormat("es-CO", { dateStyle: "long" }).format(new Date(item.published_at || item.created_at))}</p><h1 className="mt-3 text-4xl font-bold md:text-6xl">{item.title}</h1>{item.summary && <p className="mt-6 text-xl leading-relaxed text-muted-foreground">{item.summary}</p>}<div className="mt-10 whitespace-pre-line text-lg leading-relaxed">{item.content || item.summary}</div></article></main>;
}
