...
export default async function StoreAdmin({ searchParams }: { searchParams: Promise<{ editar?: string; mensaje?: string }> }) {
  ...
  return <main className="min-h-screen bg-background p-8 md:p-10"><div className="mx-auto max-w-4xl">
    ...
    <form action={saveProduct} className="mt-5 grid gap-4 border p-6">
      ...
      {item?.image_url && <div className="flex items-center gap-3 text-sm"><a href={item.image_url} target="_blank" rel="noreferrer" className="underline">Abrir imagen principal</a><form action={deleteProductMainImage}><input type="hidden" name="product_id" value={item.id} /><button type="submit" className="text-red-600 underline">Borrar imagen principal</button></form></div>}
