...
import { eventDateTimeInput, formatEventDate } from "@/lib/events";
    ...
    <main className="min-h-screen bg-background px-6 py-10 text-foreground md:px-10">
      <div className="mx-auto max-w-4xl">
        ...
        <form action={editingEvent ? updateEvent : createEvent} className="grid gap-5 rounded-lg border border-border bg-card p-6 shadow-sm">
          ...
          <div className="grid gap-5 md:grid-cols-2"><Field name="starts_at" label="Inicio (hora de Colombia)" type="datetime-local" required defaultValue={eventDateTimeInput(editingEvent?.starts_at)} /><Field name="ends_at" label="Finalización (hora de Colombia)" type="datetime-local" defaultValue={eventDateTimeInput(editingEvent?.ends_at)} /></div>
        ...
        <section id="historial" className="mt-12"><h2 className="mb-5 text-2xl font-bold">Historial de eventos</h2>
          {eventHistory?.length ? <div className="divide-y border border-border">{eventHistory.map((event) => <div key={event.id} className="flex flex-wrap items-center justify-between gap-3 p-4"><div><p className="font-bold">{event.title}</p><p className="text-xs text-muted-foreground">{formatEventDate(event.starts_at)}</p></div><div className="flex items-center gap-4"><span className={event.is_published ? "text-xs font-bold text-[#0051A2]" : "text-xs font-bold text-[#FF46A2]"}>{event.is_published ? "Publicado" : "Borrador"}</span><Link href={`/admin/eventos?editar=${event.id}`} className="text-sm underline">Editar</Link>{event.is_published && <Link href={`/eventos/${event.slug}`} className="text-sm underline">Ver</Link>}<ConfirmDeleteForm action={deleteEvent} id={event.id} message={`¿Borrar definitivamente el evento “${event.title}”? Esta acción no se puede deshacer.`} /></div></div>)}</div> : <p className="border border-border p-5 text-sm text-muted-foreground">Aún no se han creado eventos.</p>}
        </section>
      </div>
    </main>
  );
