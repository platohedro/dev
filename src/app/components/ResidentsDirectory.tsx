...
export function ResidentsDirectory({ residents }: { residents: ResidentMapItem[] }) {
  ...
  return <section className="mt-14">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold tracking-[.2em] text-[#99CC33] uppercase">Archivo vivo</p><h2 className="mt-2 text-4xl font-bold">Directorio de residentes</h2></div>{years.length > 1 && <button onClick={() => setYear("all")} className="text-sm underline">Ver todos</button>}</div>
    ...
    <div className="mt-6 border border-border bg-card p-6 md:p-8">
      <h3 className="border-b border-[#ff466f] pb-3 text-3xl font-bold">{year === "all" ? "Todos los años" : year}</h3>
      {filtered.length ? <ul className="mt-5 grid gap-x-8 md:grid-cols-2">{filtered.sort((a, b) => a.name.localeCompare(b.name)).map((resident) => <li key={resident.id} className="border-b border-border py-3">{resident.profile_url ? <a href={resident.profile_url} target="_blank" rel="noreferrer" className="font-bold underline decoration-[#ff466f] underline-offset-4">{resident.name} ↗</a> : <p className="font-bold">{resident.name}</p>}<p className="mt-1 text-sm text-muted-foreground">{resident.nationality} · {resident.country}{year === "all" ? ` · ${resident.residency_year}` : ""}</p></li>)}</ul> : <p className="text-muted-foreground">Aún no hay residentes publicados para este año.</p>}
    </div>
  </section>;
}
